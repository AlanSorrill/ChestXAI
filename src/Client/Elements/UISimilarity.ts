...
import { UI_XRay_Image, UIFrame, UIFrameResult, MainBristol, DiseaseDefinition, BristolFontFamily, BristolHAlign, BristolVAlign, linearInterp, UIButton, UICorner, UIElement, UIFrame_CornerWidthHeight, UIProgressButton } from "../ClientImports";
...
export class UISimilarityCard extends UIElement {
    ...
    setData(similarityData: [string, number, DiseaseDefinition[]]) {
        ...
        let ths = this;
        this.image.ogSrc = `patients/${similarityData[0]}`;
        this.image.setImage(`patients/${similarityData[0]}?res=20`).then(() => {
            ths.addOnAttachToBristolListener(async () => {
                ...
                for (let i = 0; i < similarityData[2].length; i++) {
                    ths.image.preloadHeatmap(similarityData[2][i].bitStringID, 100);
                }
